...
export default function JungennamenPage() {
  ...
  const boyNames = BABY_NAMES.filter((n) => n.gender === 'boy' || n.gender === 'unisex');

  const filtered = boyNames.filter((n) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16">
      ...
      <div className="max-w-3xl mx-auto text-center mb-10">
        ...
        <p className="text-base sm:text-lg text-[#777777] leading-relaxed">
          Klangvoll, charakterstark und modern: Von Spitzenreitern wie Noah und Matteo bis hin zu nordischen Lieblingen wie Fiete, Mats und Theo.
        </p>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 mb-6 flex-wrap">
        {[
          { id: 'all', label: 'Alle Jungennamen' },
          { id: 'short', label: 'Kurze Namen (3–4 Buchstaben)' },
          { id: 'classic', label: 'Klassisch & Bodenständig' },
          { id: 'rare', label: 'Nordisch & Selten' },
          { id: 'modern', label: 'Moderne Favoriten' },
        ].map((tab) => (
          <button
            key={tab.id}
